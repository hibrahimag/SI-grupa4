...
describe('GET /api/auth/verify-email', () => {
  test('200 - uspjesna verifikacija emaila', async () => {
    ...
    expect(authService.verifyEmailService).toHaveBeenCalledWith('validtoken');
  });
  ...
  test('400 - neispravan verifikacioni token', async () => {
    authService.verifyEmailService.mockRejectedValue(new Error('Neispravan verifikacioni token.'));

    const res = await request(app).get('/api/auth/verify-email?token=wrong');

    expect(res.status).toBe(400);
    expect(res.body.message).toBe('Neispravan verifikacioni token.');
  });

  test('500 - neocekivana greska verifikacije', async () => {
...
describe('POST /api/auth/resend-verification-email', () => {
  test('200 - ponovo salje verifikacioni email', async () => {
    ...
    expect(res.status).toBe(200);
    expect(authService.resendVerificationEmailService).toHaveBeenCalledWith('haris@test.com');
  });
  ...
  test('400 - nedostaje email', async () => {
    ...
    expect(res.status).toBe(400);
    expect(authService.resendVerificationEmailService).not.toHaveBeenCalled();
  });
  ...
  test('500 - neocekivana greska slanja', async () => {
    const err = new Error('SMTP crash');
    err.status = 500;
    authService.resendVerificationEmailService.mockRejectedValue(err);
    ...
    expect(res.status).toBe(500);
    expect(res.body.message).toBe('SMTP crash');
  });
});
