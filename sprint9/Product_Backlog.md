**PRODUCT BACKLOG**

### 1 - Registracija korisnika
- Opis: Omogućiti korisniku registraciju putem email adrese i lozinke  
- Tip: Feature  
- Prioritet: High  
- Procjena složenosti: 3  
- Status: Done 
- Napomena: SIGN UP  

---

### 2 - Prijava korisnika
- Opis: Omogućiti korisniku prijavu na sistem  
- Tip: Feature  
- Prioritet: High  
- Procjena složenosti: 2  
- Status: Done  
- Napomena: LOG IN  

---

### 3 - Profil studenta
- Opis: Unos osnovnih podataka (ime, prezime, indeks, godina studija, odsjek)  
- Tip: Feature  
- Prioritet: High  
- Procjena složenosti: 3  
- Status: Done  

---

### 4 - Profil kompanije
- Opis: Kreiranje profila kompanije za objavljivanje oglasa  
- Tip: Feature  
- Prioritet: High  
- Procjena složenosti: 3  
- Status: Done  

---

### 5 - Pristup koordinatora i administratora
- Opis: Omogućiti koordinatoru pristup sa posebnim privilegijama, a administratoru mogućnost upravljanja korisničkim računima  
- Tip: Feature  
- Prioritet: High  
- Procjena složenosti: 2  
- Status: Done 

---

### 6 - Kreiranje oglasa
- Opis: Kompanija kreira oglas sa opisom, trajanjem, brojem mjesta i uslovima  
- Tip: Feature  
- Prioritet: High  
- Procjena složenosti: 5  
- Status: Done  

---

### 7 - Pregled oglasa
- Opis: Student može pregledati listu dostupnih praksi  
- Tip: Feature  
- Prioritet: High  
- Procjena složenosti: 3  
- Status: Done  

---

### 8 - Detalji oglasa
- Opis: Student može vidjeti detalje pojedinačne prakse  
- Tip: Feature  
- Prioritet: Medium  
- Procjena složenosti: 2  
- Status: Done  

---

### 9 - Prijava na praksu
- Opis: Student se prijavljuje na praksu  
- Tip: Feature  
- Prioritet: High  
- Procjena složenosti: 3  
- Status: Done  

---

### 10 - Upload dokumentacije
- Opis: Student prilaže CV i motivaciono pismo  
- Tip: Feature  
- Prioritet: High  
- Procjena složenosti: 3  
- Status: Done  

---

### 11 - Pregled prijava
- Opis: Kompanija pregledava prijave studenata  
- Tip: Feature  
- Prioritet: High  
- Procjena složenosti: 3  
- Status: Done  

---

### 12 - Selekcija kandidata
- Opis: Kompanija evidentira uži izbor kandidata  
- Tip: Feature  
- Prioritet: High  
- Procjena složenosti: 3  
- Status: Done  

---

### 13 - Odobravanje prakse
- Opis: Koordinator odobrava praksu  
- Tip: Feature  
- Prioritet: High  
- Procjena složenosti: 2  
- Status: Done  

---

### 14 - Potvrda studenta
- Opis: Student potvrđuje učešće na praksi  
- Tip: Feature  
- Prioritet: High  
- Procjena složenosti: 1  
- Status: To Do  

---

### 15 - Postavljanje baze podataka
- Opis: Dizajnirati i implementirati bazu podataka za korisnike, oglase i prijave  
- Tip: Technical Task  
- Prioritet: High  
- Procjena složenosti: 5  
- Status: Done 

---

### 16 - Implementacija autentifikacije
- Opis: Implementirati login i registraciju (backend i sigurnost)  
- Tip: Technical Task  
- Prioritet: High  
- Procjena složenosti: 5  
- Status: Done  

---

### 17 - Generisanje ugovora
- Opis: Sistem automatski generiše ugovor o praksi  
- Tip: Feature  
- Prioritet: Medium  
- Procjena složenosti: 5  
- Status: To Do  

---

### 18 - Preuzimanje ugovora
- Opis: Student preuzima ugovor o praksi  
- Tip: Feature  
- Prioritet: Medium  
- Procjena složenosti: 2  
- Status: To Do  

---

### 19 - Evidencija aktivnosti
- Opis: Student unosi dnevne ili sedmične aktivnosti  
- Tip: Feature  
- Prioritet: Medium  
- Procjena složenosti: 5  
- Status: To Do  

---

### 20 - Praćenje prisustva
- Opis: Kompanija evidentira prisustvo studenta  
- Tip: Feature  
- Prioritet: Medium  
- Procjena složenosti: 3  
- Status: To Do  

---

### 21 - Evaluacija studenta
- Opis: Kompanija evaluira studenta prema definisanim kriterijima  
- Tip: Feature  
- Prioritet: Medium  
- Procjena složenosti: 3  
- Status: To Do  

---

### 22 - Evaluacija kompanije
- Opis: Student evaluira kompaniju  
- Tip: Feature  
- Prioritet: Medium  
- Procjena složenosti: 2  
- Status: To Do  

---

### 23 - Izvještaji
- Opis: Kompanija generiše izvještaje o praksama  
- Tip: Feature  
- Prioritet: Medium  
- Procjena složenosti: 5  
- Status: To Do  

---

### 24 - Analiza postojećih rješenja
- Opis: Istražiti postojeće sisteme za upravljanje praksama  
- Tip: Research  
- Prioritet: Medium  
- Procjena složenosti: 2  
- Status: Done 

---

### 25 - Dokumentacija sistema
- Opis: Dokumentovati funkcionalnosti i strukturu sistema  
- Tip: Documentation  
- Prioritet: Medium  
- Procjena složenosti: 3  
- Status: Done  

---

### 26 - Uređivanje oglasa
- Opis: Kompanija može izmijeniti postojeći aktivni oglas  
- Tip: Feature  
- Prioritet: Medium  
- Procjena složenosti: 3  
- Status: Done  

---

### 27 - Odustajanje od prakse
- Opis: Student može odustati od prijavljene ili odobrene prakse  
- Tip: Feature  
- Prioritet: Medium  
- Procjena složenosti: 2  
- Status: To Do  

---

### 28 - Obnavljanje lozinke
- Opis: Korisnik može resetovati lozinku putem email linka  
- Tip: Feature  
- Prioritet: High  
- Procjena složenosti: 3  
- Status: To Do  

---

### 29 - Student dashboard
- Opis: Centralizovani pregled svih prijava i statusa za studenta  
- Tip: Feature  
- Prioritet: High  
- Procjena složenosti: 3  
- Status: Done  

---

### 30 - Filtriranje i pretraživanje oglasa
- Opis: Student može filtrirati oglase po kriterijima i pretraživati po ključnoj riječi  
- Tip: Feature  
- Prioritet: Medium  
- Procjena složenosti: 3  
- Status: Done  

---

### 31 - Notifikacije
- Opis: Sistem šalje notifikacije korisnicima o promjenama statusa prijava  
- Tip: Feature  
- Prioritet: Medium  
- Procjena složenosti: 5  
- Status: Done  

---

### 32 - Verifikacija email adrese
- Opis: Sistem šalje verifikacioni email nakon registracije  
- Tip: Feature  
- Prioritet: High  
- Procjena složenosti: 3  
- Status: Done  

---

### 33 - Deaktivacija/brisanje korisničkog računa
- Opis: Korisnik može deaktivirati ili obrisati svoj nalog  
- Tip: Feature  
- Prioritet: Low  
- Procjena složenosti: 3
- Status: Done  

---

### 34 - Landing page
- Opis: Implementacija početne stranice sa opisom platforme i opcijama za registraciju/prijavu  
- Tip: Feature  
- Prioritet: High  
- Procjena složenosti: 3
- Status: Done  

---

### 35 - Navigacija
- Opis: Implementacija navigacije prilagođene korisničkoj roli  
- Tip: Feature  
- Prioritet: High  
- Procjena složenosti: 2
- Status: Done  

---

### 36 - Pregled korisničkog profila
- Opis: Korisnik može vidjeti detalje tj. pohranjene informacije o svom profilu
- Tip: Feature  
- Prioritet: Medium  
- Procjena složenosti: 2
- Status: Done  

---

### 37 - Privacy policy & User Terms stranica
- Opis: Stranica na kojoj su napisani uslovi korištenja
- Tip: Feature  
- Prioritet: Low
- Procjena složenosti: 2
- Status: Done 

---

### 38 - Tamni režim rada
- Opis: Implementacija tamnog režima rada aplikacije  
- Tip: Feature  
- Prioritet: Low
- Procjena složenosti: 2
- Status: Done

---

### 39 - Uređivanje korisničkog profila
- Opis: Korisnik može izmijeniti neke podatke svog profila
- Tip: Feature  
- Prioritet: Medium
- Procjena složenosti: 2
- Status: Done

---

### 40 - Arhiviranje oglasa
- Opis: Kompanija može arhivirati stare oglase kako bi zadržala evidenciju bez prikazivanja studentima
- Tip: Feature  
- Prioritet: Low
- Procjena složenosti: 2
- Status: Done

---

### 41 - Pregled statistike prijava
- Opis: Kompanija može vidjeti statistiku prijava na oglas kako bi bolje razumjela interes studenata
- Tip: Feature  
- Prioritet: Medium
- Procjena složenosti: 3
- Status: Done

---

### 42 - Historija aktivnosti (Audit log)
- Opis: Administrator ima uvid u historiju svih ključnih akcija u sistemu
- Tip: Feature  
- Prioritet: High
- Procjena složenosti: 5
- Status: Done
